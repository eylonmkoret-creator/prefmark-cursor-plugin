...
- To propose an **Investment Memo section edit**, also send `section_key` (`exec`, `thesis`, `biz`, `team`, `market`, `traction`, `raise`, or `key_risk`) and `proposed_text`. For `key_risk`, `target` is the existing risk name. Do not send `claims` with a section edit. Confirm the proposed text before sending. PrefMark polishes the wording without changing facts and returns `proposed_text`: show the investor that version, since it is exactly what lands on Accept.
